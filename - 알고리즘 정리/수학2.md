...
### 카탈란 수
> 카탈란 수(catalan number)로 불리는 수열이 있다. 핀란드 수학자 카탈란의 이름이 붙힌 이 수열을 기호로는 `Cn = 2nCn-2nCn+1`으로 나타낸다.
> 여러가지 문제들을 풀이하는 과정에서 나타난다.

1. 여는 괄호 n개와 닫는 괄호 n개로 이루어진 올바른 괄호 문자열의 개수 `Cn`
2. 항 n+1개를 곱하는 순서의 수 `Cn`
	- ((ab)c)d (a(bc))d (ab)(cd) a((bc)d) a(b(cd))
3. 단말 정점이 n+1개인 풀 바이너리티 트리의 개수 (풀 바이너리티 트리: 자식이 2개 or 0개)
